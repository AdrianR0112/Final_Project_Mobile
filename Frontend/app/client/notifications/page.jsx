    ...
    <section className="mx-auto max-w-[1280px] px-5 py-8 md:px-10">
      <div className="mb-6 flex items-end justify-between gap-4">
        <div>
          ...
          <h1 className="section-title mt-2">Notificaciones</h1>
        </div>
            ...
            <article key={notification.id} className={`rounded-3xl border p-5 ${notification.read ? 'border-[#c3c5d9] bg-white' : 'border-[#9db8ff] bg-[#eff4ff]'}`}>
              <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                <div>
                  ...
                  {notification.serviceId ? <p className="mt-2 text-xs text-[#737688]">Servicio #{notification.serviceId}</p> : null}
                </div>
                <div className="flex gap-3 md:flex-col md:items-end">
                  ...
                  {notification.actionUrl ? <Link href={notification.actionUrl} className="text-sm font-semibold text-[#003ec7]">Abrir</Link> : null}
